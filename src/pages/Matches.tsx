...
import { ArrowLeft, Heart, User, Star } from "lucide-react";
...
const Matches = ({ cachedData }: MatchesProps) => {
        ...
        <div className="flex items-start space-x-4">
          <div className="relative">
            <Avatar className="w-24 h-24 ring-2 ring-white/20 group-hover:ring-white/40 transition-all duration-300">
              ...
              />
              <AvatarFallback className="bg-gradient-to-br from-pink-500 to-red-500 text-white font-semibold text-xl">
                {user.name?.charAt(0) || <User className="w-10 h-10" />}
              </AvatarFallback>
            </Avatar>
            <div className="absolute -inset-1 bg-gradient-to-br from-pink-400 to-red-400 rounded-full blur opacity-0 group-hover:opacity-30 transition-opacity duration-300"></div>
          </div>
  ...
  const EmptyState = () => (
    <div className="text-center py-20">
      ...
      <h3 className="text-xl font-semibold text-white/90 mb-3">No matches yet</h3>
      <p className="text-white/60 max-w-md mx-auto leading-relaxed">Keep swiping to find your perfect match! When someone likes you back, they'll appear here.</p>
    </div>
  );
